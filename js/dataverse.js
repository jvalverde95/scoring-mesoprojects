...
async function dvApi(method, path, token, body=null) {
  // Route through /api/dataverse Vercel proxy to avoid CORS.
  // DV_URL lives in the server — never called directly from the browser.
  const headers = {
  ...
  // Ask DV to return the created record on POST (needed to get the new GUID)
  if (method === 'POST') headers['Prefer'] = 'return=representation';

  const opts = { method, headers };
  if (body) opts.body = JSON.stringify(body);

  const res = await fetch('/api/dataverse', opts);
  return res;
}
